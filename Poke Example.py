# dictionary with pokemon type advantages, first type in value is stronger than second in pair
...
class Pokemon():
    ...
    def lose_health(self, lose_health):
        ...
        if self.health > 0:
            print(
                "{name} lose health! Curent health brings: {health} points".format(name=self.name, health=self.health))
        else:
            self.knocked_out()
    ...
    def gain_health(self, add_health):
        ...
        if self.knocked_down == True:
            ...
            print("{name} recovered +{h} health points, current have {health} health points".format(h=add_health,
                                                                                                    name=self.name,
                                                                                                    health=self.health))
        else:
            self.health = self.max_health
            print("{name} recovered MAX health points, current have {health} health points".format(name=self.name,
                                                                                                   health=self.health))
...
class Trainer():
    ...
    def attack_other_trainer(self,
                             other_trainer):  # attack between current pokemons, belonging to two different trainers
        if self.current_pokemon != "N/A" and other_trainer.current_pokemon != "N/A":
            print("{t1} pick {p1} and attack {t2} who pick to defence a {p2}".format(t1=self.name,
                                                                                     p1=self.current_pokemon.name,
                                                                                     t2=other_trainer.name,
